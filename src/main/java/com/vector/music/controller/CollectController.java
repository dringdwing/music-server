...
import com.vector.music.domain.Collect;
import com.vector.music.service.CollectService;
...
public class CollectController {
     ...
     */
    @PostMapping("/addList")
    public Object addCollectList(HttpServletRequest request){
        JSONObject jsonObject = new JSONObject();
        String userId = request.getParameter("userId");           //用户id
        String type = request.getParameter("type");               //收藏类型（0歌曲1歌单）
        ...
        if(CollectService.existSongId(Integer.parseInt(userId),Integer.parseInt(songListId))){
            jsonObject.put(Consts.CODE,2);
            jsonObject.put(Consts.MSG,"歌单已收藏");
            return jsonObject;
        }
        ...
        Collect.setUserId(Integer.parseInt(userId));
        Collect.setType(new Byte(type));
        Collect.setSongId(Integer.parseInt(songListId));
        boolean flag = CollectService.insert(Collect);
        if(flag){   //保存成功
            jsonObject.put(Consts.CODE,1);
            jsonObject.put(Consts.MSG,"收藏成功");
